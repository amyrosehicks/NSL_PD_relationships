...
ukbec_enrich <- as.data.frame(getModulesEnrichment(ukbec_net, 
...
OLG_mod_enrich_fig <- as.data.frame(t(OLG_mod_enrich_fig)) %>%
  rownames_to_column(var = "Module")
ukbec_enrich_fig <- ukbec_enrich_NSL %>%
  dplyr::filter_all(any_vars(.<0.05)) 
...
# Round pvalue just for presentation purposes
OLG_NSL_figure$`-log10(p.value)` <- signif(OLG_NSL_figure$`-log10(p.value)`, digits = 3)
# Produce plot
ggplot(OLG_NSL_figure, aes(Gene, Cell_type)) +
  ...
  scale_fill_gradient(low = "white", high = "goldenrod3", limits = c(0,1.5)) +
  labs(y = "Cell type") +
  theme(
