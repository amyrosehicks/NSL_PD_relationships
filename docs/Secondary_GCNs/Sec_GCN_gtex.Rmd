...
big_figure3 <- big_figure2 %>%
  ...
  dplyr::mutate(FDR = p.adjust(Enrichment, method = "fdr")) %>%
...
ggplot(big_figure3, aes(Gene, Cell_type)) +
  geom_tile(aes(fill = `-log10(FDR)`)) +
  geom_text(aes(label = signif.num2(FDR)), size = 3) +
  scale_fill_gradient(low = "white", high = "goldenrod3", limits = c(0,15.5)) +
  labs(y = "Cell type", fill = "-log10(FDR)") +
  facet_grid(factor(Network, 
                    levels = c("None", 
                               "Astrocyte-corrected", 
                               "Microglia-corrected", 
...
## PD genes 
Second, the overall clustering of the PD associated genes was examined, focusing on modules containing *KAT8* and *KANSL1*.
```{r}
# Join together PD gene list enrichment tables, filter for KAT8/KANSL1 modules
N_PD_mendelian_mod2 <- N_PD_mendelian_mod1 %>%
  dplyr::mutate(List = "Mendelian PD") %>%
  dplyr::filter(module %in% c("darkslateblue", "salmon4"))
N_PD_GWAS_mod2 <- N_PD_GWAS_mod1 %>%
  dplyr::mutate(List = "Sporadic PD") %>%
  dplyr::filter(module %in% c("darkslateblue", "salmon4"))
N_PD_mod <- dplyr::bind_rows(N_PD_mendelian_mod2, N_PD_GWAS_mod2)
N_PD_mod$Network <- "Neuron-corrected"
MA_PD_mendelian_mod2 <- MA_PD_mendelian_mod1 %>%
  dplyr::mutate(List = "Mendelian PD") %>%
  dplyr::filter(module %in% c("cyan", "red"))
MA_PD_GWAS_mod2 <- MA_PD_GWAS_mod1 %>%
  dplyr::mutate(List = "Sporadic PD") %>%
  dplyr::filter(module %in% c("cyan", "red"))
MA_PD_mod <- dplyr::bind_rows(MA_PD_mendelian_mod2, MA_PD_GWAS_mod2)
MA_PD_mod$Network <- "Astrocyte-corrected"
MG_PD_mendelian_mod2 <- MG_PD_mendelian_mod1 %>%
  dplyr::mutate(List = "Mendelian PD") %>%
  dplyr::filter(module %in% c("red", "magenta"))
MG_PD_GWAS_mod2 <- MG_PD_GWAS_mod1 %>%
  dplyr::mutate(List = "Sporadic PD") %>%
  dplyr::filter(module %in% c("red", "magenta"))
MG_PD_mod <- dplyr::bind_rows(MG_PD_mendelian_mod2, MG_PD_GWAS_mod2)
MG_PD_mod$Network <- "Microglia-corrected"
OLG_PD_mendelian_mod2 <- OLG_PD_mendelian_mod1 %>%
  dplyr::mutate(List = "Mendelian PD") %>%
  dplyr::filter(module %in% c("green", "black"))
OLG_PD_GWAS_mod2 <- OLG_PD_GWAS_mod1 %>%
  dplyr::mutate(List = "Sporadic PD") %>%
  dplyr::filter(module %in% c("green", "black"))
OLG_PD_mod <- dplyr::bind_rows(OLG_PD_mendelian_mod2, OLG_PD_GWAS_mod2)
...
PD_fig$FDR <- as.numeric(PD_fig$FDR)
# Alter names 
PD_fig$module[PD_fig$module=="salmon4"] <- "salmon4 (KAT8)"
PD_fig$module[PD_fig$module=="darkslateblue"] <- "darkslateblue (KANSL1)"
PD_fig$module[PD_fig$module=="red" & PD_fig$Network == "Astrocyte-corrected"] <- "red (KAT8)"
PD_fig$module[PD_fig$module=="cyan"] <- "cyan (KANSL1)"
PD_fig$module[PD_fig$module=="magenta"] <- "magenta (KAT8)"
PD_fig$module[PD_fig$module=="red"& PD_fig$Network == "Microglia-corrected"] <- "red (KANSL1)"
PD_fig$module[PD_fig$module=="green"] <- "green (KANSL1)"
PD_fig$module[PD_fig$module=="black"] <- "black (KAT8)"
```
...
OLG_PD_fig <- OLG_PD_mod %>%
  dplyr::select(List, module, Network) %>% 
  dplyr::rename(Module = module)
PD_mendelian_rep$List <- "Mendelian PD"
PD_GWAS_rep$List <- "Sporadic PD"
prim_PD_rep <- bind_rows(PD_mendelian_rep, PD_GWAS_rep)
