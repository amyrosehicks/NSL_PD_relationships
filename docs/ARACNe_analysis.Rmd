...
gsp = function(){ return(file.path(here::here("docs")))}
...
### NSL regulon
Genes appearing in 3 or more of the regulons were focused on, termed the **NSL regulon**. 
```{r}
...
The number of unique genes predicted to be regulated by 3 or more NSL complex genes is: `r nrow(unique(gt_3regs$Target))`.

## 4.2. Gene set enrichment analysis {.tabset}
### Individual regulons
The appearance of each PD gene list in the regulons were tested for significance using a **Fishers exact test**. If the overlap p-value **<0.05**, this meant these gene list appeared as targets for these two genes more often than would be expected by chance.
```{r}
# Make summary dataframe
...
### NSL regulon
The NSL regulon was also filtered for genes linked to Mendelian PD.
```{r}
# Filter
gt_3regs_mendPD <- gt_3regs %>%
  dplyr::filter(Target %in% PD_mendelian_list$Ensembl_ID) %>%
  dplyr::mutate(Target_gene = as.character(Target))
# Add target gene symbols
for (i in PD_mendelian_list$Ensembl_ID) {
  loc <- match(i, PD_mendelian_list$Ensembl_ID)
  gt_3regs_mendPD$Target_gene[gt_3regs_mendPD$Target_gene == i] <- PD_mendelian_list$Gene_symbol[loc]
}
# Show results
gt_3regs_mendPD %>%
  dplyr::select(Regulator_gene, Target_gene, MI, MI_quantile)
```

`r length(unique(gt_3regs_mendPD$Target_gene))` different genes associated with Mendelian PD were present in the NSL regulon. This was tested for significance using a **Fishers exact test**.
```{r}
fishers_test_lists2(c(3,4,5,6), c(PD_mendelian_list$Ensembl_ID), gt_regs)
```

The NSL regulon was also filtered for genes linked to sporadic PD. 
```{r}
# Filter
gt_3regs_gwasPD <- gt_3regs %>%
  dplyr::filter(Target %in% PD_GWAS_list$Ensembl_ID) %>%
  dplyr::mutate(Target_gene = as.character(Target))
# Add target gene symbols
for (i in PD_GWAS_list$Ensembl_ID) {
  loc <- match(i, PD_GWAS_list$Ensembl_ID)
  gt_3regs_gwasPD$Target_gene[gt_3regs_gwasPD$Target_gene == i] <- PD_GWAS_list$Gene_symbol[loc]
}
# Show results
gt_3regs_gwasPD %>%
  dplyr::select(Regulator_gene, Target_gene, MI, MI_quantile)
...
`r length(unique(gt_3regs_gwasPD$Target_gene))` different genes associated with sporadic PD were present in the NSL regulon. This was tested for significance using a **Fishers exact test**.
```{r}
fishers_test_lists2(c(3,4,5,6), c(PD_GWAS_list$Ensembl_ID), gt_regs)
```
...
### NSL regulon
The NSL regulon was was analysed in a similar way to the individual regulons. 
```{r}
# Filter results for genes in 3+ regulons
gt_regs_count <- gt_aracne %>%
  dplyr::count(Target)
...
## 4.4. KEGG/reactome term entichment {.tabset}
### Individual regulons
...
The number of unique genes predicted to be regulated by 3 or more NSL complex genes is: `r nrow(unique(uk_3regs$Target))`.

## 5.2. Gene set enrichment analysis {.tabset}
### Individual regulons
```{r}
...
uk_aracne_PDmend <- uk_aracne %>%
  dplyr::filter(Target_gene %in% PD_mendelian_list$UKBEC)
```
...
uk_aracne_GWAS <- uk_aracne %>%
  dplyr::filter(Target_gene %in% PD_GWAS_list$UKBEC)
```

The appearance of each gene list in the results for the remaining NSL genes within the **black** module were tested for significance using a **Fishers exact test**. If the overlap p-value **<0.05**, this meant these gene list appeared as targets for these two genes more often than would be expected by chance. 
```{r}
# Make summary dataframe
...
```{r}
# Filter
uk_3regs_mendPD <- uk_3regs %>%
  dplyr::filter(Target %in% PD_mendelian_list$Ensembl_ID) %>%
  dplyr::mutate(Target_gene = as.character(Target))
# Add target gene symbols
for (i in PD_mendelian_list$Ensembl_ID) {
  loc <- match(i, PD_mendelian_list$Ensembl_ID)
  uk_3regs_mendPD$Target_gene[uk_3regs_mendPD$Target_gene == i] <- PD_mendelian_list$Gene_symbol[loc]
}
# Show results
uk_3regs_mendPD %>%
  dplyr::select(Regulator_gene, Target_gene, MI, MI_quantile)
...
`r length(unique(uk_3regs_mendPD$Target_gene))` different genes associated with Mendelian PD were present in the NSL regulon. This was tested for significance using a **Fishers exact test**.
```{r}
fishers_test_lists2(c(3,4,5,6), c(PD_mendelian_list$Ensembl_ID), uk_regs)
```

The NSL regulon was also filtered for genes linked to sporadic PD. 
```{r}
# Filter
uk_3regs_gwasPD <- uk_3regs %>%
  dplyr::filter(Target %in% PD_GWAS_list$Ensembl_ID) %>%
  dplyr::mutate(Target_gene = as.character(Target))
# Add target gene symbols
for (i in PD_GWAS_list$Ensembl_ID) {
  loc <- match(i, PD_GWAS_list$Ensembl_ID)
  uk_3regs_gwasPD$Target_gene[uk_3regs_gwasPD$Target_gene == i] <- PD_GWAS_list$Gene_symbol[loc]
}
# Show results
uk_3regs_gwasPD %>%
  dplyr::select(Regulator_gene, Target_gene, MI, MI_quantile)
...
`r length(unique(uk_3regs_gwasPD$Target_gene))` different genes associated with sporadic PD were present in the NSL regulon. This was tested for significance using a **Fishers exact test**.
```{r}
fishers_test_lists2(c(3,4,5,6), c(PD_GWAS_list$Ensembl_ID), uk_regs)
```

## 5.3. GO term enrichment analysis {.tabset}
### Individual regulons
...
### NSL regulon
The NSL regulon was analysed in a similar way to the individual regulons. 
```{r, eval = FALSE}
# Run gprofiler
...
## 5.4. KEGG/reactome term enrichment analysis {.tabset}
### Individual regulons 
These terms were then compared to the list if PD-linked terms from [Bandres-Ciga et al., 2020](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC8096770/). Firstly, as this list only contained terms from KEGG, Reactome and Pathway Interaction Database sources, the `gProfiler` results for each regulon were filtered for these sources and formatted. 
```{r}
# Filter for KEGG and REACTOME terms
...
The list of terms enriched for each NSL regulon was then filtered for the PD-linked terms. 
```{r}
# Filter ARACNe list for paper list
...
### NSL regulon
The KEGG and reactome term enrichments for the NSL regulon was also compared to this PD-linked list of terms. 
```{r}
# Filter for KEGG and REACTOME terms
