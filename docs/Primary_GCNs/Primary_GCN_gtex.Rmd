...
NSL_report_gt <- reportOnGenes(tissue = "FCortex", genes = NSL_genes, which.one = "gtexv6", ens.correction = c("ENSG00000257382", "ENSG00000120071"))
# Select report
...
NSL_BP = e[e$query.number %in% NSL_gt_modules & e$domain == "BP",]
NSL_MF = e[e$query.number %in% NSL_gt_modules & e$domain == "MF",]
NSL_CC = e[e$query.number %in% NSL_gt_modules & e$domain == "CC",]
NSL_BP_2 <- dplyr::select(NSL_BP, query.number, term.id, term.name, domain, p.value)
NSL_MF_2 <- dplyr::select(NSL_MF, query.number, term.id, term.name, domain, p.value)
NSL_CC_2 <- dplyr::select(NSL_CC, query.number, term.id, term.name, domain, p.value)
# Rename columns
names(NSL_BP_2)[names(NSL_BP_2)=="query.number"] <- "gene_list"
names(NSL_BP_2)[names(NSL_BP_2)=="term.id"] <- "go_id"
names(NSL_BP_2)[names(NSL_BP_2)=="domain"] <- "go_type"
names(NSL_BP_2)[names(NSL_BP_2)=="term.name"] <- "go_term"
names(NSL_MF_2)[names(NSL_MF_2)=="query.number"] <- "gene_list"
names(NSL_MF_2)[names(NSL_MF_2)=="term.id"] <- "go_id"
names(NSL_MF_2)[names(NSL_MF_2)=="domain"] <- "go_type"
names(NSL_MF_2)[names(NSL_MF_2)=="term.name"] <- "go_term"
names(NSL_CC_2)[names(NSL_CC_2)=="query.number"] <- "gene_list"
names(NSL_CC_2)[names(NSL_CC_2)=="term.id"] <- "go_id"
names(NSL_CC_2)[names(NSL_CC_2)=="domain"] <- "go_type"
names(NSL_CC_2)[names(NSL_CC_2)=="term.name"] <- "go_term"
# FDR correct p values
NSL_BP_2$p.value <- as.numeric(NSL_BP_2$p.value)
NSL_BP_2$FDR <- p.adjust(NSL_BP_2$p.value, method="fdr")
NSL_MF_2$p.value <- as.numeric(NSL_MF_2$p.value)
NSL_MF_2$FDR <- p.adjust(NSL_MF_2$p.value, method="fdr")
NSL_CC_2$p.value <- as.numeric(NSL_CC_2$p.value)
NSL_CC_2$FDR <- p.adjust(NSL_CC_2$p.value, method="fdr")
```
...
NSL_BP_GO <- go_reduce(pathway_df = NSL_BP_2, threshold = 0.7, scores = NULL)
NSL_MF_GO <- go_reduce(pathway_df = NSL_MF_2, threshold = 0.7, scores = NULL)
NSL_CC_GO <- go_reduce(pathway_df = NSL_CC_2, threshold = 0.7, scores = NULL)
```

The GO term enrichments annotated to these modules were extracted and simplified according to parent terms, then plotted.
```{r, warning = FALSE}
# GO plot for biological processes
go_plot(NSL_BP_GO, col_gene_list = "gene_list", col_fdr = "FDR", fct_gene_list = NSL_gt_modules)
# GO plot for molecular functions
go_plot(NSL_MF_GO, col_gene_list = "gene_list", col_fdr = "FDR", fct_gene_list = NSL_gt_modules)
# GO plot for cellular components
go_plot(NSL_CC_GO, col_gene_list = "gene_list", col_fdr = "FDR", fct_gene_list = NSL_gt_modules)
```
...
PD_mendelian_report2_gt <- reportOnGenes(tissue = "FCortex", genes = PD_mendelian_gt_genes, which.one = "gtexv6", ens.correction = c("ENSG00000262446", "ENSG00000177628", "ENSG00000257437", "ENSG00000186868"))
# Select report
...
```{r}
# Convert report into dataframe
PD_mendelian_rep2_gt <- data.frame(matrix(unlist(PD_mendelian_rep2_gt), nrow = PD_mendelian_rep2_gt_length, byrow = FALSE), stringsAsFactors=FALSE)
...
```{r, results = FALSE, warning = FALSE}
# Again search genes of interest in specified tissue/network 
PD_GWAS_report2_gt <- reportOnGenes(tissue = "FCortex", genes = PD_GWAS_gt_genes, which.one = "gtexv6", ens.correction = c("ENSG00000263031", "ENSG00000145723", "ENSG00000233209", "ENSG00000179344", "ENSG00000228987", "ENSG00000204287", "ENSG00000257437", "ENSG00000186868", "ENSG00000262234", "ENSG00000145725",  "ENSG00000255439", "ENSG00000167397", "ENSG00000232748", "ENSG00000167394"))
# Select report
...
```{r}
# Convert report into dataframe
PD_GWAS_rep2_gt <- data.frame(matrix(unlist(PD_GWAS_rep2_gt), nrow = PD_GWAS_rep2_gt_length, byrow = FALSE), stringsAsFactors=FALSE)
