...
CoExpNets::plotModSizes(tissue="FCTX", which.one="10UKBEC")
```
...
check_synonyms("FCTX", "10UKBEC", NSL_genes, NSL_rep_UK$gene)
```
...
```{r, results = FALSE, warning = FALSE}
# Search for gene list in dataset
NSL_report2_UK <- reportOnGenes(tissue = "FCTX", genes = NSL_UK_genes, which.one = "10UKBEC")
# Select report
...
NSL_rep2_UK <- data.frame(matrix(unlist(NSL_rep2_UK), nrow = NSL_rep2_UK_length, byrow = FALSE), stringsAsFactors=FALSE)
...
names(NSL_rep2_UK)[2] <- "ENSG"
# Order by p value
NSL_rep2_UK <- NSL_rep2_UK[order(NSL_rep2_UK$Fisher), ]
# FDR correct
...
e2 = read.csv(file.path(here::here("data", "CoExp_source", "netFCTX.12.signed.it.20.rds_cor_pca.rds_gprofiler.csv")), 
             stringsAsFactors = FALSE)
# Extract all GO term enrichments from source file for modules of interest
NSL_UK_BP = e2[e2$query.number %in% NSL_UK_modules & e2$domain == "BP",]
NSL_UK_MF = e2[e2$query.number %in% NSL_UK_modules & e2$domain == "MF",]
NSL_UK_CC = e2[e2$query.number %in% NSL_UK_modules & e2$domain == "CC",]
NSL_UK_BP_2 <- dplyr::select(NSL_UK_BP, query.number, term.id, term.name, domain, p.value)
NSL_UK_MF_2 <- dplyr::select(NSL_UK_MF, query.number, term.id, term.name, domain, p.value)
NSL_UK_CC_2 <- dplyr::select(NSL_UK_CC, query.number, term.id, term.name, domain, p.value)
#NSL_terms <- dplyr::bind_rows(NSL_BP_2, NSL_MF_2, NSL_CC_2)
# Rename columns
names(NSL_UK_BP_2)[names(NSL_UK_BP_2)=="query.number"] <- "gene_list"
names(NSL_UK_BP_2)[names(NSL_UK_BP_2)=="term.id"] <- "go_id"
names(NSL_UK_BP_2)[names(NSL_UK_BP_2)=="domain"] <- "go_type"
names(NSL_UK_BP_2)[names(NSL_UK_BP_2)=="term.name"] <- "go_term"
names(NSL_UK_MF_2)[names(NSL_UK_MF_2)=="query.number"] <- "gene_list"
names(NSL_UK_MF_2)[names(NSL_UK_MF_2)=="term.id"] <- "go_id"
names(NSL_UK_MF_2)[names(NSL_UK_MF_2)=="domain"] <- "go_type"
names(NSL_UK_MF_2)[names(NSL_UK_MF_2)=="term.name"] <- "go_term"
names(NSL_UK_CC_2)[names(NSL_UK_CC_2)=="query.number"] <- "gene_list"
names(NSL_UK_CC_2)[names(NSL_UK_CC_2)=="term.id"] <- "go_id"
names(NSL_UK_CC_2)[names(NSL_UK_CC_2)=="domain"] <- "go_type"
names(NSL_UK_CC_2)[names(NSL_UK_CC_2)=="term.name"] <- "go_term"
# FDR correct p values
NSL_UK_BP_2$p.value <- as.numeric(NSL_UK_BP_2$p.value)
NSL_UK_BP_2$FDR <- p.adjust(NSL_UK_BP_2$p.value, method="fdr")
NSL_UK_MF_2$p.value <- as.numeric(NSL_UK_MF_2$p.value)
NSL_UK_MF_2$FDR <- p.adjust(NSL_UK_MF_2$p.value, method="fdr")
NSL_UK_CC_2$p.value <- as.numeric(NSL_UK_CC_2$p.value)
NSL_UK_CC_2$FDR <- p.adjust(NSL_UK_CC_2$p.value, method="fdr")
```

```{r, results = FALSE, warning = FALSE, message = FALSE}
# GO reduce
NSL_UK_BP_GO <- go_reduce(pathway_df = NSL_UK_BP_2, threshold = 0.7, scores = NULL)
NSL_UK_MF_GO <- go_reduce(pathway_df = NSL_UK_MF_2, threshold = 0.7, scores = NULL)
NSL_UK_CC_GO <- go_reduce(pathway_df = NSL_UK_CC_2, threshold = 0.7, scores = NULL)
```

The GO term enrichments attached to these modules were extracted and simplified according to parent terms, then plotted.
```{r, warning = FALSE}
#  GO plot for biological processes
go_plot(NSL_UK_BP_GO, col_gene_list = "gene_list", col_fdr = "FDR", fct_gene_list = NSL_UK_modules)
#  GO plot for molecular functions
go_plot(NSL_UK_MF_GO, col_gene_list = "gene_list", col_fdr = "FDR", fct_gene_list = NSL_UK_modules)
#  GO plot for cellular components
go_plot(NSL_UK_CC_GO, col_gene_list = "gene_list", col_fdr = "FDR", fct_gene_list = NSL_UK_modules)
``` 
...
PD_mendelian_report_UK <- reportOnGenes(tissue = "FCTX", genes = PD_mendelian_genes, which.one = "10UKBEC")
# Select report
...
check_synonyms("FCTX", "10UKBEC", PD_mendelian_genes, PD_mendelian_rep_UK$gene)
```
...
PD_mendelian_report2_UK <- reportOnGenes(tissue = "FCTX", genes = PD_mendelian_UK_genes, which.one = "10UKBEC")
# Select report
...
```{r}
# Convert report into dataframe
PD_mendelian_rep2_UK <- data.frame(matrix(unlist(PD_mendelian_rep2_UK), nrow = PD_mendelian_rep2_UK_length, byrow = FALSE), stringsAsFactors=FALSE)
...
# Order by p value
PD_mendelian_rep2_UK <- PD_mendelian_rep2_UK[order(PD_mendelian_rep2_UK$Fisher), ]
```
...
PD_mendelian_NSL_mod_UK[, c(-6, -7, -8)]
```
...
```{r, results = FALSE, warning = FALSE}
# Search genes of interest in specified tissue/network 
PD_GWAS_report_UK <- reportOnGenes(tissue = "FCTX", genes = PD_GWAS_genes, which.one = "10UKBEC")
# Select report
...
# Check if any synonyms of the genes are present in networks
check_synonyms("FCTX", "10UKBEC", PD_GWAS_genes, PD_GWAS_rep_UK$gene)
```
...
```{r, results = FALSE, warning = FALSE}
# Again search genes of interest in specified tissue/network 
PD_GWAS_report2_UK <- reportOnGenes(tissue = "FCTX", genes = PD_GWAS_UK_genes, which.one = "10UKBEC")
# Select report
...
```{r}
# Convert report into dataframe
PD_GWAS_rep2_UK <- data.frame(matrix(unlist(PD_GWAS_rep2_UK), nrow = PD_GWAS_rep2_UK_length, byrow = FALSE), stringsAsFactors=FALSE)
...
# Order by p value
PD_GWAS_rep2_UK <- PD_GWAS_rep2_UK[order(PD_GWAS_rep2_UK$Fisher), ]
```
...
ggplot(melt_summ_UK, aes(Gene_list, Module)) +
  geom_tile(aes(fill = `-log10(FDR)`)) +
  geom_text(aes(label = signif.num(FDR))) +
  scale_fill_gradient(low = "white", high = "lightsteelblue4", limits = c(0,3.6)) +
